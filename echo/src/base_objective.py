...
class BaseObjective:
    ...
    def __init__(self, config, metric="val_loss", device="cpu"):
        ...
        self.results_fn = os.path.join(
            save_path, f"hyper_opt_{random.randint(0, 1e5)}.csv")
        while os.path.isfile(self.results_fn):
            rand_index = random.randint(0, 1e5)
            self.results_fn = os.path.join(
                save_path, f"hyper_opt_{rand_index}.csv")

        logger.info(
            f"Initialized an objective to be optimized with metric {metric}")
        logger.info(f"Using device {device}")
        logger.info(
            f"Saving study/trial results to local file {self.results_fn}")
